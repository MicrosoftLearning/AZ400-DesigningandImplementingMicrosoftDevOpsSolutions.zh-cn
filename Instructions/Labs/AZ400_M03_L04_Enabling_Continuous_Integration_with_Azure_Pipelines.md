...
7. 单击“保存”按钮，保存管道定义
8. 管道将采用基于项目名称的名称。 让我们重命名它，以便更好地识别管道。 转到“管道 > 管道”，然后单击最近创建的管道。 单击省略号和“重命名/删除”选项。 将其命名为 eshoponweb-ci-pr，然后单击“保存”。 
...
3. 单击“Feature01”，导航到作为 Feature01 分支的一部分的 /eShopOnWeb/src/Web/Program.cs 文件，并在第一行进行以下更改： 
...
4. 单击“提交>提交”（保留默认提交消息）。
5. 此时会弹出一条消息，建议创建拉取请求（因为 Feature01 分支现在的改动比主分支要多）。  单击“创建拉取请求”。
6. 在“新建拉取请求”选项卡中，保留默认值并单击“创建”。 
7. 拉取请求将根据应用于目标主分支的策略显示一些挂起的要求。
    - 至少 1 个用户应查看并批准更改。
    - 生成验证，你将看到生成 eshoponweb-ci-pr 已自动触发

8. 在所有验证都成功后，单击右上角的“批准”。 现在，从“设置自动完成”下拉列表中，可以单击“完成”。 
9. 在“完成拉取请求”选项卡上，单击“完成合并” 
...
1. 现在，需要将“trigger: none”代码替换为以下代码：
...
2. 单击“保存”按钮（不是“保存并运行”），保存管道定义。 
3. 选择“为此提交创建新分支”
4. 保留默认分支名称并选中“启动拉取请求”。
5. 单击“保存”
6. 管道将采用基于项目名称的名称。 让我们重命名它，以便更好地识别管道。 转到“管道 > 管道”，然后单击最近创建的管道。 单击省略号和“重命名/删除”选项。 将其命名为 eshoponweb-ci，然后单击“保存”。 
7. 转到“Repos”>“拉取请求”
8. 单击现有的拉取请求
9. 在所有验证都成功后，单击右上角的“批准”。 现在，可以单击“完成”。
10. 在“完成拉取请求”选项卡上，单击“完成合并” 
...
3. 单击新的“Feature02”分支
4. 导航到“/eShopOnWeb/src/Web/Program.cs”文件，并删除第一行：
...
5. 单击“提交>提交”（保留默认提交消息）。
6. 此时会弹出一条消息，建议创建拉取请求（因为 Feature02 分支现在的改动比主分支要多）。 
7. 单击“创建拉取请求”。
...
12. 返回“管道”>“管道”，你会注意到在代码合并后，自动触发了生成 eshoponweb-ci。 
13. 单击“eshoponweb-ci”生成，然后选择最后的运行。
14. 成功执行后，单击“相关”>“已发布”，检查已发布的项目：
    - Bicep：基础结构项目。
    - 网站：应用项目。
