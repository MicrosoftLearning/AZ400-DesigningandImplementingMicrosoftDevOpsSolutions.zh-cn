...
1. 单击“设置为默认分支”。
    ...
    > **** 注意：如果主分支已是默认分支，则“设置为默认分支”**** 选项灰显。在这种情况下，请按照说明继续操作
...
1. 如果系统提示选择“Bash”或“PowerShell”，请选择“Bash”。
   ...
   > **注意**：如果这是你第一次启动“Cloud Shell”，并看到“入门”弹出窗口，请选择“不需要存储帐户”，然后选择你在此实验室中使用的订阅，并单击“应用”。****************
...
1. 若要创建 Windows App 服务计划，请运行以下命令：
   ...
   ```
    ...
    > **注意**：如果在运行上一个命令时出现“订阅未注册为使用命名空间'Microsoft.Web'”之类的错误，请运行以下命令 `az provider register --namespace Microsoft.Web`，然后再次执行之前出错的命令。
...
1. 在“Azure 应用服务部署”窗格中，指定以下设置，并单击“添加”：
   ...
   - 打开“应用程序和配置设置”部分，并在“应用设置”文本框中添加 `-UseOnlyInMemoryDatabase true -ASPNETCORE_ENVIRONMENT Development`********
...
1. 如果 YAML 缩进处于禁用状态，则在编辑器中仍选中已添加的任务，按两次 Tab 键以将其缩进四个空格****。

   > **注意**：建议在前后也各添加一个空行，使其更易阅读。

1. 单击“验证并保存”，在“验证并保存”窗格上，再次单击“保存”，以直接将更改提交到主分支************。
...
1. 从“管道”视图中可以看到这一点，其中显示“正在等待(1 个正在检查中)”****。 同时，还会显示一条通知消息，指示“在此运行可以继续部署到 Azure Web 应用之前，需要对 1 个审批进行审阅”****。
1. 单击此消息旁边的“审阅”**** 按钮。
1. 在出现的“正在等待审阅”**** 窗格中，单击“审批”**** 按钮。
