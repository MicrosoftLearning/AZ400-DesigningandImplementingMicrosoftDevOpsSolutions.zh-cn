...
4. 在 Cloud Shell 窗格中的 Bash 提示符下，运行以下命令以创建资源组（将 `<region>` 占位符替换为离你最近的 Azure 区域的名称，例如“centralus”、“westeurope”或其他选择的区域） 。
...
7. 关闭 Azure Cloud Shell，但让 Azure 门户在浏览器中保持打开状态。
...
4. 在“选择存储库”窗格中，单击“eShopOnWeb_MultiStageYAML” 。
...
2. 在显示 eShopOnWeb_MultiStageYAML/.ado/eshoponweb-ci.yml 文件内容的窗格中，导航到文件末尾（第 56 行），然后按 Enter/Return 添加新的空行。
3. 在第 57 行上，添加以下内容，定义 YAML 管道中的“发布”阶段 。
...
9. 验证任务是否列为“steps”任务的子任务。 如果没有，则选择添加的任务中的所有行，按两次 Tab 键以将其缩进四个空格，以便将该任务作为“steps”任务的子项列出 。
...
10. 将光标置于“部署”阶段的“steps” 节点下的第一行上，然后按 Enter/Return 添加一个新的空行（第 64 行）。
11. 在“任务”窗格上，搜索并选择“下载生成项目”任务 。
12. 为此任务指定以下参数：
    - 下载以下作业生成的项目：当前生成
    - 下载类型：特定项目
...
15. 如果 YAML 缩进处于禁用状态，则在编辑器中仍选中添加的任务，按两次 Tab 键以将其缩进四个空格。
...
17. 在 Azure DevOps 左侧菜单中，导航到“管道”，然后再次选择“管道”。
18. 打开“EShopOnWeb_MultiStageYAML”管道，然后单击“运行管道”。
19. 从显示的窗格中确认“运行”。
20. 请注意，此时出现了两个不同的阶段，即“生成 .Net Core 解决方案”和“部署到 Azure Web 应用”。
21. 等待管道启动，并等待它成功完成生成阶段。
22. 一旦部署阶段需要启动，系统会提示你“需要权限”，并显示一个橙色条：
...
23. 单击“查看”
24. 在“正在等待审阅”窗格中，单击“允许”。
25. 验证“允许弹出”窗口中的消息，然后单击“允许”进行确认。
26. 这将启动部署阶段。 等待该过程成功完成。
...
2. 在左侧的“管道菜单”下，选择“环境”。
3. 单击“创建环境”。
4. 在“新建环境”窗格中，为环境添加一个名称“approvals”。
5. 在“资源”下，选择“无”。
6. 单击“创建”按钮确认设置。
7. 环境创建完成后，单击“添加资源”按钮旁边的“省略号”(...)。
8. 选择“审批和检查”。
9. 在“添加你的第一个检查”中，选择“审批”。
10. 将 Azure DevOps 用户帐户名称添加到“approvers”字段。

    > 注意：在现实场景中，这将显示为处理此项目的 DevOps 团队的名称。

11. 单击“创建”按钮确认已定义的审批设置。
12. 最后，我们需要将必需的“environment: approvals”设置添加到部署阶段的 YAML 管道代码中。 为此，导航到“Repos”，浏览到“.ado”文件夹，然后选择“eshoponweb-ci.yml”管道即代码文件。
13. 在“内容”视图中，单击“编辑”按钮切换到编辑模式。
14. 导航到“部署作业”的起始位置（第 60 行的 -job: Deploy）
15. 在下方添加一个新的空行，并添加以下代码片段：
...
16. 由于环境是部署阶段的特定设置，因此“jobs”不能使用它。 我们必须对当前的作业定义进行一些额外的更改。
17. 在第 60 行，将“- job: Deploy”重命名为“- deployment: Deploy”
18. 接下来，在第 63 行 (vmImage: Windows-2019) 下，添加一个新的空行。
19. 粘贴以下 YAML 代码片段：
...
20. 选择其余的代码片段（第 67 行一直到末尾），并使用 Tab 键固定 YAML 的缩进 。
...
21. 单击“提交”，然后在出现的“提交”窗格中再次单击“提交”，确认对代码 YAML 文件的更改。
22. 导航到左侧的“Azure DevOps 项目”菜单，选择“管道”，接着选择“管道”，然后注意前面使用的“EshopOnWeb_MultiStageYAML”管道。
23. 打开管道。
24. 单击“运行管道”以触发新的管道运行；单击“运行”进行确认。
25. 与之前一样，生成阶段按预期启动。 等待启动过程成功完成。
26. 接下来，由于我们已经为部署阶段配置了“environment:approvals”，它将在启动之前请求审批确认。
27. 从“管道”视图中可以看到这一点，其中显示“正在等待(已通过 0/1 项检查)”。 同时，还会显示一条通知消息，指示“在此运行可以继续部署到 Azure Web 应用之前，审批需要评审”。
28. 单击此消息旁边的“查看”按钮。
29. 在显示的“部署到 Azure Web 应用的检查和手动验证”窗格中，单击“审批等待”消息。
30. 单击“批准”。
31. 这样，部署阶段就可以启动并成功部署 Azure Web 应用源代码了。
