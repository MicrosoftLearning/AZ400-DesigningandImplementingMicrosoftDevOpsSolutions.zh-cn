...
- 如果尚未安装 Git 2.47.0 或更高版本，请启动 Web 浏览器，导航到[适用于 Windows 的 Git 下载页面](https://gitforwindows.org/)以进行下载和安装。
...
#### 任务 1：配置 Git 和 Visual Studio Code
...
#### 任务 2：（如果已完成，请跳过此任务）创建和配置团队项目

在此任务中，你将创建一个 eShopOnWeb Azure DevOps 项目，供多个实验室使用。

1. 在实验室计算机上，在浏览器窗口中打开 Azure DevOps 组织。 单击“新建项目”。 将项目命名为 eShopOnWeb，然后在“工作项进程”下拉列表中选择“Scrum”。 单击“创建”。

    ![“新建项目”面板的屏幕截图。](images/create-project.png)

#### 任务 3：（如果已完成，请跳过此任务）导入 eShopOnWeb Git 存储库
...
在本练习中，你将使用 Visual Studio Code 将更改提交到 eShopOnWeb**** 存储库的主**** 分支。

> **备注**：主**** 分支是 eShopOnWeb**** 存储库中的默认分支，是用于实验室其余部分的分支。
...
1. 在“eShopOnWeb”存储库窗格的右上角，单击“克隆”。
    ...
    ![克隆 Git 存储库的屏幕截图。](images/clone-repo.png)
...
1. 在命令面板提示符下，运行“Git:**Clone”命令**。
    ...
    ![VS Code 命令面板的屏幕截图。](images/vscode-command.png)
...
1. 在“源代码管理”**** 选项卡处于选中状态的情况下，在窗格顶部的文本框中，键入“`My commit`”**** 作为提交消息，然后按 Ctrl+Enter**** 在本地进行提交。

    ![第一次提交的屏幕截图。](images/first-commit.png)
...
1. 验证提交是否显示在列表的顶部。
    ...
    ![ADO 存储库提交的屏幕截图。](images/ado-commit.png)
...
1. 在“源代码管理”**** 选项卡处于选中状态的情况下，在窗格顶部的文本框中，键入“`Added comments`”**** 作为提交消息 。

    ![暂存更改的屏幕截图。](images/staged-changes.png)
...
1. 在 Visual Studio Code 窗口的“源代码管理”选项卡处于打开状态时，选择代表该文件的非暂存版本的 Constants.cs。
    ...
    ![文件比较的屏幕截图。](images/file-comparison.png)
...
1. 单击省略号，在下拉菜单中，选择“浏览文件”，然后查看结果。
    ...
    ![提交浏览的屏幕截图。](images/commit-browse.png)
...
1. 在弹出窗口中，选择“+ 从其中创建新分支…”。
    ...
    ![“创建分支”选项的屏幕截图。](images/create-branch.png)

1. 在“选择引用以从中创建分支”文本框中，选择“master”作为引用分支。********
1. 在“分支名称”**** 文本框中，键入“`dev`”**** 以指定新分支，然后按 Enter****。
...
1. 单击省略号，在弹出菜单中选择“删除分支”，并在出现确认提示时单击“删除” 。
    ...
    ![“删除分支”选项的屏幕截图。](images/delete-branch.png)
...
1. 单击弹出菜单中的省略号，选择“分支策略”。
    ...
    ![“分支策略”选项的屏幕截图。](images/branch-policies.png)
...
1. 在存储库设置的 main 选项卡上，启用“检查链接的工作项”选项，并将其保留为“必需”选项。
    ...
    ![策略设置的屏幕截图。](images/policy-settings.png)
...
1. 单击“提交 > 提交”。 你将看到一条警告：只能使用拉取请求完成对主分支的更改。
    ...
    ![策略被拒绝提交的屏幕截图。](images/policy-denied.png)
...
1. 此时会弹出一条消息，建议创建拉取请求（因为与主分支相比，开发分支现已在更改方面领先）。 单击“创建拉取请求”。
    ...
    ![“创建拉取请求”选项的屏幕截图。](images/create-pr.png)
...
1. 在右侧选项中，单击“工作项”旁边的 + 按钮。 单击之前创建的工作项将其链接到拉取请求。 你将看到其中一个要求更改状态。
    ...
    ![链接工作项的屏幕截图。](images/link-wit.png)
...
1. 在“标记”窗格中，单击“新建标记” 。
1. 在“创建标记”**** 面板的“名称”**** 文本框中，键入“`v1.1.0-beta`”****，在“基于”**** 下拉列表中，保留“主”**** 条目处于选中状态，在“说明”**** 文本框中键入“`Beta release v1.1.0`”****，然后单击“创建”****。
...
1. 在存储库设置的“**主**”选项卡上，禁用“**检查链接的工作项**”选项。
    ...
    ![分支策略的屏幕截图。](images/branch-policies.png)

1. 现已禁用/移除主分支的分支策略。

## 审阅

在本实验室中，你已了解如何在 Azure Repos 中使用 Git 进行版本控制。
