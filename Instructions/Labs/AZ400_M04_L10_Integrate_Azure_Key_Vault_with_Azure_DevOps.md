...
- **** 完成实验室环境验证：在开始此实验室之前，请确保已完成[验证实验室环境](AZ400_M00_Validate_lab_environment.md)，该步骤会设置本实验室所需的 Azure DevOps 组织、项目和服务连接。
...
1. 单击“保存并运行”，等待管道成功执行。 你可能需要再次单击“保存并运行”****，以完成管道创建和运行过程。

    > 重要说明：如果看到消息“此管道需要访问资源的权限，然后才能继续运行 Docker Compose to ACI”，请再次单击“查看”、“允许”和“允许”。 这是允许管道创建资源所必需的。 必须单击生成作业才能看到“权限”消息。
...
1. 在“创建密钥保管库”边栏选项卡的“访问配置”选项卡上，选择“保管库访问策略”，然后在“访问策略”部分中，单击“+ 创建”以设置新策略    。
    ...
    > **注意**：你需要保护对密钥保管库的访问，只允许得到授权的应用程序和用户进行访问。 若要从保管库访问数据，需要为在实验室环境验证期间创建的服务连接提供读取（获取/列出）权限，以便在管道中进行身份验证。

    1. 在“权限”边栏选项卡上“机密权限”的下方，选中“获取”和“列出”权限。 单击“下一步”。
    2. 在“主体”**** 边栏选项卡上，搜索“Azure 订阅服务连接”****（实验室环境验证期间创建的连接，通常命名为“azure subs”），并从列表中选择它。 可以在 Azure DevOps 中的“项目设置”>“服务连接”>“azure subs”>“管理服务主体”下找到服务主体名称。 如果在选择 Azure 订阅时遇到权限错误，请单击“授权”**** 按钮，此操作会自动在密钥保管库中为你创建访问策略。 单击“下一步”、“下一步”、“创建”（访问策略）  。
...
1. 单击“保存并运行”。 你可能需要再次单击“保存并运行”****，以完成管道创建和运行过程。 必须单击生成作业才能看到任何“权限”消息。
1. 打开管道并等待管道成功执行。
...
1. 若要验证管道部署的结果，请在 Azure 门户中搜索并选择“AZ400-EWebShop-NAME”**** 资源组。 在资源列表中，确认管道已创建“az400eshop”**** 容器实例。
